...
std::vector<std::pair<int, int>> CTextureSynthesizer<Scalar_t, Channel>::__buildNeighborOffset(int vKernelSize)
{
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
void CTextureSynthesizer<Scalar_t, Channel>::run(const Texture_t& vInput, const Eigen::MatrixXi& vMask, Texture_t& vioScene)
{
	if (vInput.cols() * vInput.rows() == 0 || vMask.size() == 0 || vioScene.cols() * vioScene.rows() == 0)
		_THROW_RUNTIME_ERROR("Error: Synthesizer input is NULL");

	if (vioScene.cols() != vMask.cols() || vioScene.rows() != vMask.rows())
		_THROW_RUNTIME_ERROR("Error: Mask can not match with Scene");

	if (vInput.data()== NULL || vMask.data()== NULL || vioScene.data()== NULL)
		_THROW_RUNTIME_ERROR("Error: Synthsizer input is NULL");
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
void CTextureSynthesizer<Scalar_t, Channel>::__initCache(const Eigen::MatrixXi& vMask, const Texture_t& vOutput)
{
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
void CTextureSynthesizer<Scalar_t, Channel>::__initInputPyramid(const Texture_t& vTexture)
{
	CMipmapGenerator<Color_t> TextureMipmapGenerator;
	TextureMipmapGenerator.setKernalSize(m_GaussianSize);
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
void CTextureSynthesizer<Scalar_t, Channel>::__initTexture(const Texture_t& vFrom, Texture_t& voTo) const
{
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
void CTextureSynthesizer<Scalar_t, Channel>::__initTextureWithNeighborMask(const Texture_t& vFrom, Texture_t& voTo) const
{
	CMipmapGenerator<Color_t> TextureMipmapGenerator;
	TextureMipmapGenerator.setKernalSize(m_GaussianSize);
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
bool CTextureSynthesizer<Scalar_t, Channel>::__increase(int& vioLayer, int& vioGeneration) const
{
...
////FUNCTION: 
template <typename Scalar_t, unsigned Channel>
bool CTextureSynthesizer<Scalar_t, Channel>::__decrease(int& vioLayer, int& vioGeneration) const
{
	bool Reduce = false;
	if (vioGeneration > 0)
		--vioGeneration;
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
void CTextureSynthesizer<Scalar_t, Channel>::__synthesizeTexture(int vLayer, int vGeneration)
{
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
auto CTextureSynthesizer<Scalar_t, Channel>::__buildInputFeatureAt(int vLayer, int vGeneration, Eigen::Index vRowId, Eigen::Index vColId) const -> Feature_t
{
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
auto CTextureSynthesizer<Scalar_t, Channel>::__buildOutputFeatureAt(int vLayer, int vGeneration, Eigen::Index vRowId, Eigen::Index vColId) const -> Feature_t
{
	if (__decrease(vLayer, vGeneration))
	{
		vRowId /= 2;
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
auto CTextureSynthesizer<Scalar_t, Channel>::__buildFeatureAt(const Texture_t& vTexture, Eigen::Index vRowId, Eigen::Index vColId) const->Feature_t
{
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
auto CTextureSynthesizer<Scalar_t, Channel>::__buildFeatureWithNeighborMask(const Texture_t& vTexture, Eigen::Index vRowId, Eigen::Index vColId, const Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic>& vMask) const->Feature_t
{
...
//FUNCTION: 
template <typename Scalar_t, unsigned Channel>
auto CTextureSynthesizer<Scalar_t, Channel>::__findNearestValue(int vLayer, int vGeneration, const Feature_t& vFeature) const -> Color_t
{	
