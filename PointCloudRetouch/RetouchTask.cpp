...
#include "PointClusterExpander.h"  //FIXME-010: 这个头文件有必要include吗？
...
bool CRetouchTask::init(const hiveConfig::CHiveConfig* vConfig)
{
	...
	try 
	{
		auto NumSubconfig = vConfig->getNumSubconfig();  //FIXME-010：为什么就这么一句话都需要try/catch？
	}
	catch (...)
	{
		throw "vConfig Error";
...
//FIXME-010：你的程序逻辑允许重复调用init()吗？如果不允许，就要保证m_pConfig为空
	m_pConfig = vConfig;  
	m_pClusterConfig = nullptr;

//FIXME-010: 下面有好几个地方都出现了_HIVE_EARLY_RETURN，即解析XML可能失败，但失败后，m_pConfig仍然存有有效值，
//        按理说，如果解析失败，m_pConfig就该复原
	for (auto i = 0; i < vConfig->getNumSubconfig(); i++)
	{
		...
		if (_IS_STR_IDENTICAL(pConfig->getSubconfigType(), std::string("CLUSTER")))
		{
			...
			{
				_HIVE_OUTPUT_WARNING("It is NOT allowed to define cluster twice.");  //FIXME-010：需要补一个测试用例，验证确实能处理连续定义两次CLUSTER的情况
			}
			continue;
		}
		if (_IS_STR_IDENTICAL(pConfig->getSubconfigType(), std::string("CLASSIFIER")))
		{
			...
			_ASSERTE(ClassifierSig.has_value());  //FIXME-010：这个地方用_ASSERTE()太弱了，如果外部配置文件就是没定义SIG，在release状态下就会出错

//FIXME-010: m_pPointClusterExpander这个命名不好，对于一个retouch task来说，这个m_pPointClusterExpander好像扯不上关系
...
//FIXME-010：需要补一个测试用例，证明能处理m_pPointClusterExpander没有创建成功的情况
...
//FIXME-010: 这里的设计有点怪，是否采用多线程，竟然会有两个完全不同的类
...
//*****************************************************************
//FUNCTION: 
//FIXME-010: 函数名说的是MarkedPoints，实际返回的是ExpandPoints
