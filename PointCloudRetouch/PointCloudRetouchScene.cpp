...
//FIXME：程序的逻辑允许两次init()吗？如果不允许，就要保证调用init()的时候，m_pPointCloudScene为空

	if (vPointCloudScene == nullptr)   //FIXME: 为什么这里对参数有效性检查不用_ASSERTE，而要用抛出异常的方式？初始化失败返回false就好，为什么要抛异常？
...
{
	_ASSERTE(vIndex < m_pPointCloudScene->size());  //FIXME: 首先要对m_pPointCloudScene的有效性做检查
...
{
	std::vector<pcl::index_t> TempPoints;  //FIXME：这个变量叫 PointsInBox 不是更好吗？
	for (auto Index = 0; Index < m_pPointCloudScene->size(); Index++)
	{
		auto& Point = m_pPointCloudScene->points[Index];
		Eigen::Vector3f Pos{ Point.x, Point.y, Point.z };  //FIXME：确定这个临时变量会不会在每次循环后都被析构（debug和release方式下都要测试）。如果要被频繁析构，这个变量就要放到循环体外
...
{//FIXME: 函数名说要提取rgba，但返回的是rgb，故意的吗？
	union ColorLayout
	{
		struct
		{
	...
	ColorLayout Color;  
	Color.rgba = vRgba;    //FIXME：这种颜色的转化方法很奇怪，给个说明文档
