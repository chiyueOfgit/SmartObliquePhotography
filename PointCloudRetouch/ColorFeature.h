...
namespace hiveObliquePhotography
{
	namespace PointCloudRetouch
	{
		...
		private:
			...
			std::vector<Eigen::Vector3i> __kMeansCluster(const std::vector<Eigen::Vector3i>& vColorSet, std::size_t vK) const;

			float __calcColorDifferences(const Eigen::Vector3i& vLColor, const Eigen::Vector3i& vRColor);
			float __calculateCIEDE2000(const LAB& lab1, const LAB& lab2);
			LAB __RGB2LAB(const Eigen::Vector3f& vRGBColor);
