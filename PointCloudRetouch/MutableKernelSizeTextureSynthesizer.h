...
namespace hiveObliquePhotography::PointCloudRetouch
{
	...
	private:
		...
		//TODO: magic number
		int m_MinKernelSize = 3;
		int m_GaussianSize = 9;
		int m_PyramidLayer = 4;
		int m_GenerationNum = 3;
		std::vector<std::vector<std::pair<int, int>>> m_NeighborOffsets;
		...
		void __initSearchSet(const Texture_t& vTexture);
		...
		Feature_t __buildFeatureAt(const Texture_t& vTexture, Eigen::Index vRowId, Eigen::Index vColId, int vLayer) const;
	...
	template class CMutableKernelSizeTextureSynthesizer<int, 3>;
	template class CMutableKernelSizeTextureSynthesizer<float, 1>;
}
