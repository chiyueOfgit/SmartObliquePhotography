...
namespace hiveObliquePhotography
{
	namespace PointCloudRetouch
	{
		...
		public:
			...
			bool executeMarker(const std::vector<pcl::index_t>& vUserMarkedRegion, double vHardness, const std::function<float(Eigen::Vector2f)>& vDistanceFunc, const Eigen::Matrix4d& vPvMatrix, EPointLabel vTargetLabel);
		...
		private:
			...
			CPointCluster* __generateInitialCluster(const std::vector<pcl::index_t>& vUserMarkedRegion, double vHardness, const std::function<float(Eigen::Vector2f)>& vDistanceFunc, const Eigen::Matrix4d& vPvMatrix, EPointLabel vTargetLabel);
