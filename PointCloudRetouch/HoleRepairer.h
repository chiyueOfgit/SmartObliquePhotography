...
namespace hiveObliquePhotography
{
	namespace PointCloudRetouch
	{
		struct SLattice
		{
			...
			Eigen::Vector3i Color{0.0f, 0.0f, 0.0f};
			float Height = 0.0f;
		};
		...
		private:
			void __generatePlaneLattices(const Eigen::Vector4f& vPlane, const std::pair<Eigen::Vector3f, Eigen::Vector3f>& vBox, const Eigen::Vector2i& vResolution, SPlaneInfos& vPlaneInfos, std::vector<std::vector<SLattice>>& voPlaneLattices);
			...
			template<class T>
			std::vector<std::vector<T>> __fillLatticesByItems(const std::vector<std::vector<T>>& vItems, std::vector<std::vector<SLattice>>& vLattices, int vOffset)
			{
