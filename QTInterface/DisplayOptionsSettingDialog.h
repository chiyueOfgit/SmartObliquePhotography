...
namespace hiveQTInterface
{
	...
	public:
			...
			m_pUi(std::make_shared<Ui::CDisplayOptionsSettingDialog>())
		{
			...
			m_pUi->ColorTest->setCheckState(*hiveObliquePhotography::AutoRetouch::CAutoRetouchConfig::getInstance()->getAttribute<bool>(KEY_WORDS::ENABLE_COLOR_TEST) ? Qt::Checked : Qt::Unchecked);
			bool sd = *hiveObliquePhotography::AutoRetouch::CAutoRetouchConfig::getInstance()->getAttribute<bool>(KEY_WORDS::ENABLE_NORMAL_TEST);
			m_pUi->GroundTest->setCheckState(*hiveObliquePhotography::AutoRetouch::CAutoRetouchConfig::getInstance()->getAttribute<bool>(KEY_WORDS::ENABLE_GROUND_TEST) ? Qt::Checked : Qt::Unchecked);
			m_pUi->NormalTest->setCheckState(*hiveObliquePhotography::AutoRetouch::CAutoRetouchConfig::getInstance()->getAttribute<bool>(KEY_WORDS::ENABLE_NORMAL_TEST) ? Qt::Checked : Qt::Unchecked);
			if (*hiveObliquePhotography::AutoRetouch::CAutoRetouchConfig::getInstance()->getAttribute<int>(KEY_WORDS::COLOR_TEST_MODE) == 0 )
